...
ネットワークの待ち行列や送受信バッファの上限を調整する設定である。アプリケーション側の backlog、
...
`somaxconn=65535` は accept 待ち行列の上限、`10000 60999` は外向き TCP 接続に使うポート範囲、
`16777216` は送受信それぞれのソケットバッファ上限である。いずれもその数の接続やバッファを
直ちに確保する設定ではない。
...
ホスト間で値が揃っていることと、`task status` で対象サービスが正常であることを確認する。
